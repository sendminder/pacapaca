...
class SendCarrotButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return TextButton.icon(
      icon: icon ?? const Icon(Icons.card_giftcard),
      label: label ?? Text('carrot.send'.tr()),
      onPressed: () async {
        ...
        if (amount != null && context.mounted) {
          ...
          try {
            final transaction = await ref
                .read(carrotSenderProvider.notifier)
                .sendCarrots(request);
